...
void mapData::catagorize_data() {
    ...
    for (unsigned intersection = 0; intersection < interNum; intersection++) {
        ...
        adjacent_intersections.resize(getNumberOfIntersections());
        adjacent_intersections[intersection] = find_adjacent_intersections(intersection);
    ...
    for (unsigned i = 0; i < POINum; i++) {
        ...
        POI_name_to_IDs[name].push_back(i);

        POIID_to_intersectionID.resize(POINum);
        LatLon position;

        position = poi_position[i];
        POIID_to_intersectionID[i] = find_closest_intersection(position);
...
void mapData::clear_osm_data() {
   ...
   // subway_line.clear();
    ...
    POI_name_to_IDs.clear();
    POIID_to_intersectionID.clear();
