...
t_transform_coordinates trans_coord;
...
static void x11_build_popup(void);
...
static void map_button(int bnum) {
    ...
    if (button_state.button[bnum].type != BUTTON_SEPARATOR) {
        ...
        button_state.button[bnum].win = XCreateSimpleWindow(
                ...
                x11_convert_to_xcolor(t_color::predef_colors[BUTTONS])
                );
...
void create_button(const char *prev_button_text, const char *button_text,
    ...
    for (i = 0; i < button_state.num_buttons; i++) {
        //printf("%s",(char*)(button_state.button[i].text));
...
bool drawtext_in(const t_bound_box& bbox, const std::string& text) {
    bool draw = drawtext(bbox.get_center(), text, bbox);
    return draw;
}

bool drawtext_in(const t_bound_box& bbox, const std::string& text, float tolerance) {
    bool draw = drawtext(bbox.get_center(), text, bbox, tolerance);
    return draw;
}
...
bool drawtext(
    ...
    bool draw = drawtext(text_center, text, tolerance_bounds);
    return draw;
}
...
bool drawtext(const t_point& text_center, const std::string& text, const t_bound_box& bounds) {
    ...
    min_bounds *= 2;
    bool draw = drawtext(text_center, text, min_bounds.x, min_bounds.y);
    return draw;
}

bool drawtext(const t_point& text_center, const std::string& text, float boundx, float boundy) {
    bool draw = drawtext(text_center.x, text_center.y, text, boundx, boundy);
    return draw;
}
...
close_graphics(void) {
    ...
    //XftDrawDestroy(x11_state.popuparea_draw);
...
build_default_menu(void) {
    ...
    x11_state.menu = XCreateSimpleWindow(
            ...
            x11_convert_to_xcolor(t_color::predef_colors[BLACK]),
            x11_convert_to_xcolor(t_color(255, 255, 255, 255 * 1 / 3))
            );
...
/* draws UTF-8 text left at xc, yc */
void leftaligntext(XftDraw* draw, int xc, int yc, const char *text) {
    int len, width;

    font_ptr menu_font = gl_state.font_info.get_font_info(15, 0);
...
static void x11_build_popup(void) {
    XSetWindowAttributes menu_attributes;
    unsigned long valuemask;
    x11_state.popup_lines = 0;

    x11_state.popup = XCreateSimpleWindow(x11_state.display, x11_state.toplevel, 10,
            trans_coord.ytop + 10, 320, 60, 0, x11_convert_to_xcolor(t_color::predef_colors[BLACK]),
            x11_convert_to_xcolor(t_color::predef_colors[WHITE]));

    x11_state.popuparea_draw = XftDrawCreate(
            x11_state.display,
            x11_state.popup,
            x11_state.visual_info.visual,
            x11_state.colormap_to_use
    ...
    menu_attributes.win_gravity = SouthWestGravity;
    valuemask = CWWinGravity | CWEventMask | CWDontPropagate;
    XChangeWindowAttributes(x11_state.display, x11_state.popup, valuemask, &menu_attributes);
    XMapWindow(x11_state.display, x11_state.popup);
}

void
clear_popup(void) {
    XClearWindow(x11_state.display, x11_state.popup);
    x11_state.popup_lines = 0;
}

void
draw_message_popup(void) {
    int savefontsize, height, width;
    height = 10; // makes the text print a line lower
    width = 15;
    t_color savecolor;
    float ylow;

    for (int i = 0; i < x11_state.popup_lines; i++) {
        //height += 15; // makes the text print a line lower
        width += 7;
    }
    x11_state.popup_lines++;
    if (gl_state.disp_type == SCREEN) {
#ifdef X11
        //XClearWindow(x11_state.display, x11_state.popup);
        ...
        XSetForeground(x11_state.display, x11_state.gc_menus,
                x11_convert_to_xcolor(t_color::predef_colors[WHITE]));
        XDrawRectangle(x11_state.display, x11_state.popup, x11_state.gc_menus, 0, 0,
                trans_coord.top_width - MWIDTH, 4 * T_AREA_HEIGHT);
        XSetForeground(x11_state.display, x11_state.gc_menus,
                x11_convert_to_xcolor(t_color::predef_colors[BLACK]));

        //        setcolor(DARKGREY);
        leftaligntext(
                x11_state.popuparea_draw,
                width,
                (T_AREA_HEIGHT / 2) + height,
                gl_state.popupMessage
                );
#else
        if (!InvalidateRect(win32_state.hStatusWnd, NULL, TRUE))
            WIN32_DRAW_ERROR();
...
/* Changes the message to be displayed on screen.   */
void
update_message_popup(const string& msg) {
    strncpy(gl_state.popupMessage, msg.c_str(), BUFSIZE);
    draw_message_popup();
#ifdef X11
    // Make this appear immediately.  Win32 does that automaticaly.
    XFlush(x11_state.display);
